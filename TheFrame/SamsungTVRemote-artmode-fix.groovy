...
def getArtModeStatus() { // library marker davegut.samsungTvWebsocket, line 82
	if (getDataValue("frameTv") == "true") { // library marker davegut.samsungTvWebsocket, line 83
		...
		//	Retry every 30s (up to 5 attempts) until the TV confirms.
		...
		if (!state.artModeWs) { // library marker davegut.samsungTvWebsocket, line 89
			def retries = (state.artModeRetry ?: 0) as Integer
			if (retries < 5) {
				state.artModeRetry = retries + 1
				runIn(30, getArtModeStatus)
			} else {
				state.artModeRetry = 0
				logWarn("getArtModeStatus: no response after 5 attempts — TV may not support art mode WS")
			}
		} else {
			state.artModeRetry = 0 // reset on success
